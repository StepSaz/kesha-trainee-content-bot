...
- **What:** Haiku-driven `handleCommentReply` gets a third tool `consult_advisor({ question, draft_answer? })` that calls Sonnet (`claude-sonnet-5`) for a short opinion on hard cases (sarcasm, ambiguous tone, facts beyond the post). The base model stays Haiku; the advisor only fires when Haiku decides it's stuck. Inspired by the Anthropic advisor strategy blog post.
